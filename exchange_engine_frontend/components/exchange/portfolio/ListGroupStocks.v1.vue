  ...
  <v-list-item-group color="">
    ...
    >
      ...
      <v-list-item-content>
        ...
        <v-list-item-subtitle v-text="security.count + ' шт.'" />
      </v-list-item-content>

      <v-list-item-action>
        <p class="text-overline ma-0">
          {{ security.stock.price | numeral('0,0.00') }}₮
        </p>
        <p class="text-caption ma-0 green--text darken-1">
          <v-icon x-small color="green">
            mdi-arrow-up
          </v-icon> {{ (security.stock.price - security.aver_price) | numeral('0,0.00') }}₮
          ({{ ((security.stock.price - security.aver_price) / security.stock.price * 100) | numeral('0,0.00') }}%)
        </p>
      </v-list-item-action>
    </v-list-item>
    <v-divider />
